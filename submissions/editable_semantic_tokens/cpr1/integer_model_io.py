...
def _deserialize_self_compressed(model, raw: bytes) -> None:
    ...
    modules = [
        module for module in model.modules() if isinstance(module, COMPRESSIBLE_TYPES)
    ]
    ...
    with torch.no_grad():
        for module in modules:
            ...
            for index, (bits, template) in enumerate(
                zip(module_depths, _weight_rows(module, parameter))
            ):
                ...
                if bits:
                    ...
                    rows = (
                        weight_bits[bit_offset : bit_offset + count_bits]
                        .reshape(count, bits)
                        .astype(np.int16)
                    )
                    unsigned = (rows * (1 << np.arange(bits, dtype=np.int16))).sum(
        ...
        for name, parameter in model.named_parameters():
            ...
            value = (
                np.frombuffer(view[:byte_count], dtype=dtype, count=parameter.numel())
                .copy()
