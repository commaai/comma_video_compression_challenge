...
# Archive format version. 1 = brotli decoder + brotli latents (original).
# 2 = adaptive range-coded decoder + latents (codec_rc), lossless, ~1.1 KB
# smaller on our trained weights. The version byte lets inflate dispatch and
# keeps v1 archives decodable.
ARCHIVE_VERSION = 2


def build_archive(decoder_state_dict, latents, meta_dict, version=ARCHIVE_VERSION):
    ...
    Layout:
      ...
      [latents_blob_len:u32] [latents_blob]
    """
    import json
    ...
    if version == 2:
        import codec_rc
        decoder_blob = codec_rc.encode_decoder_rc(q_sd)
        latents_blob = codec_rc.encode_latents_rc(latents)
    else:
        decoder_blob = encode_decoder(q_sd)
        latents_blob = brotli.compress(encode_latents(latents), quality=11)
    ...
    out.write(struct.pack("<I", len(latents_blob))); out.write(latents_blob)
    return out.getvalue()
...
def parse_archive(archive_bytes):
    ...
    dec_data = buf.read(dec_len)
    lat_len = struct.unpack("<I", buf.read(4))[0]
    lat_data = buf.read(lat_len)
    if version == 2:
        import codec_rc
        decoder_sd = codec_rc.decode_decoder_rc(dec_data)
        latents = codec_rc.decode_latents_rc(lat_data)
    else:
        decoder_sd = decode_decoder(dec_data)
        latents = decode_latents(brotli.decompress(lat_data))
