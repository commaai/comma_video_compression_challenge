    ...
    *,
    ...
    if device_name == "cuda":
        if not torch.cuda.is_available():
            raise InflationError("CUDA unavailable; the production default requires a CUDA GPU")
    elif device_name == "mps":
        if not torch.backends.mps.is_available():
            raise InflationError("MPS unavailable on this host")
    else:
        raise InflationError("device must be cuda or mps")
