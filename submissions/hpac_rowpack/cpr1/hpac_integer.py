...
def exact_power_of_two(exponent: torch.Tensor) -> torch.Tensor:
    """Preserve dyadic scales: MPS pow may perturb exact half-integer ties."""
    if exponent.device.type == "mps":
        return torch.pow(2.0, exponent.cpu()).to(exponent.device)
...
class IntegerNormGate(nn.Module):
    ...
    def forward(self, value: torch.Tensor) -> torch.Tensor:
        normalized = integer_channel_norm(value, "power", self.activation_bound)
        gate = ste_round(self.gate.clamp(0, self.gate_bound)).view(1, -1, 1, 1)
        mixed = value * (16 - gate) + normalized * gate
        return requantize(mixed, 4, -self.activation_bound, self.activation_bound)


def patch_group_mask(kernel: int, delta: int, type_: str) -> torch.Tensor:
    if type_ not in {"A", "B"}:
        raise ValueError(f"unsupported mask type: {type_}")
...
class IntegerConv2d(nn.Module):
    ...
    def codes(self):
        weight = (
            ste_round(self.weight.clamp(-self.weight_bound, self.weight_bound))
            * self.mask
        )
    ...
    def forward(self, value):
        ...
        result = F.conv2d(
            value,
            weight,
            None if exponent is not None else bias,
            padding=self.padding,
            dilation=self.dilation,
            groups=self.groups,
        )
        if exponent is not None:
            result = result * exact_power_of_two(exponent).view(1, -1, 1, 1)
...
class IntegerLinear(nn.Module):
    ...
    def codes(self):
        weight = ste_round(self.weight.clamp(-self.weight_bound, self.weight_bound))
        bias = ste_round(self.bias.clamp(-32768, 32767))
        exponent = None
        if hasattr(self, "exponent"):
            exponent = ste_round(self.exponent.clamp(self.exponent_min, 0))
    ...
    def forward(self, value):
        ...
        if exponent is not None:
            result = result * exact_power_of_two(exponent).view(1, -1)
...
class IntegerHPAC(nn.Module):
    def __init__(
        ...
        self.conv_a = IntegerConv2d(
            ...
            7,
            padding=3,
            mask=patch_group_mask(7, delta, "A"),
            **conv_kwargs,
        )
        self.conv_b1 = IntegerConv2d(
            ...
            dilation=2,
            groups=channels,
            mask=patch_group_mask(5, delta, "B"),
            **conv_kwargs,
        )
        self.conv_b2 = IntegerConv2d(
            ...
            dilation=4,
            groups=channels,
            mask=patch_group_mask(3, delta, "B"),
            **conv_kwargs,
        )
    ...
    def _to_patches(self, value):
        ...
        value = (
            value.view(batch, channels, patch_rows, self.P, patch_cols, self.P)
            .permute(0, 2, 4, 1, 3, 5)
            .contiguous()
        )
    ...
    def _from_patches(self, value, batch: int, patch_rows: int, patch_cols: int):
        ...
        value = (
            value.view(batch, patch_rows, patch_cols, channels, self.P, self.P)
            .permute(0, 3, 1, 4, 2, 5)
            .contiguous()
        )
    ...
    def prepare_frame_context(self, idx, previous_raw):
        ...
        shift = requantize(
            self.frame_shift(embedding),
            1,
            -self.activation_bound,
            self.activation_bound,
        )
        shift = (
            shift.view(batch, 1, self.ch, 1, 1)
            .expand(batch, patch_count, self.ch, 1, 1)
            .reshape(batch * patch_count, self.ch, 1, 1)
        )
        previous_one_hot = (
            F.one_hot(previous_raw, num_classes=self.num_classes)
            .permute(0, 3, 1, 2)
            .float()
        )
        past = requantize(
            self.conv_past(previous_one_hot),
            0,
            -self.activation_bound,
            self.activation_bound,
        )
        ...
        if self.use_spm:
            ...
            spm = (
                pooled.unsqueeze(3)
                .unsqueeze(5)
                .expand(batch, self.ch, patch_rows, self.P, patch_cols, self.P)
                .contiguous()
                .view(batch, self.ch, height, width)
            )
        scale = None
        if self.use_frame_scale:
            scale = requantize(self.frame_scale(embedding), 4, -8, 8)
            scale = (
                scale.view(batch, 1, self.ch, 1, 1)
                .expand(batch, patch_count, self.ch, 1, 1)
                .reshape(batch * patch_count, self.ch, 1, 1)
            )
        return (
            shift,
            self._to_patches(past),
            scale,
    ...
    def cached_context_logits(self, current, context):
        ...
        one_hot = (
            F.one_hot(current, num_classes=self.num_classes).permute(0, 3, 1, 2).float()
        )
        patches = self._to_patches(one_hot)
        coords = self._patch_coord_grid(batch * patch_count, current.device)
        hidden = requantize(
            self.conv_a(torch.cat([patches, coords], dim=1)),
            1,
            -self.activation_bound,
            self.activation_bound,
        )
        ...
        hidden = integer_channel_norm(hidden, self.norm_mode, self.activation_bound)
        hidden = integer_activation(
            requantize(
