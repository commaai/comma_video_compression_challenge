...
def collapse_chroma(x, mode):
    if mode == "normal": return x
    k = {"soft": 1, "medium": 2, "strong": 4}.get(mode, 1)
    uv = x[:, 1:3]
    uv = F.avg_pool2d(uv, kernel_size=k * 2 + 1, stride=1, padding=k)
    x[:, 1:3] = uv
    return x

def apply_luma_denoise(x, strength):
    ...
    y = x[:, 0:1]
    y_blur = F.conv2d(y, kernel_2d.to(x.device), padding=kernel_size // 2)
    blend = min(0.9, strength / 3.0)
    x[:, 0:1] = (1 - blend) * y + blend * y_blur
    return x

def rgb_to_yuv_local(rgb):
...
def segment_polygon(frame_idx, width, height):
    # Driving-focused ROI polygons
    segments = [
...
def build_mask(frame_idx, width, height, feather_radius):
...
def process_frame(frame_rgb, frame_idx, outside_luma_denoise, outside_chroma_mode, feather_radius, outside_blend):
...
def main():
    ...
    parser.add_argument("--outside-luma-denoise", type=float, default=2.0)
    parser.add_argument("--outside-chroma-mode", type=str, default="medium")
    parser.add_argument("--feather-radius", type=int, default=32)
    parser.add_argument("--outside-blend", type=float, default=0.8)
    ...
    for frame_idx, frame in enumerate(in_container.decode(in_stream)):
        ...
        out_rgb = process_frame(rgb, frame_idx, args.outside_luma_denoise, args.outside_chroma_mode, args.feather_radius, args.outside_blend)
