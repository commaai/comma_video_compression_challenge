...
from .context_warp import parse_choices, warp_torch
...
def _decode_models(models: bytes) -> tuple[bytes, bytes, bytes, object]:
    ...
    if cap1_end >= len(models):
        raise ResidualArchiveError("truncated F24S carrier, warp block or selector")
    cap1 = _restore_cap1(models[offset:cap1_end])
    try:
        warp_choices, warp_bytes = parse_choices(models[cap1_end:])
    except ValueError as error:
        raise ResidualArchiveError(f"invalid context warp block: {error}") from error
    selector = SPARSE_SELECTOR_PREFIX + models[cap1_end + warp_bytes :]
    try:
    ...
    return semantic, carrier, hpac, warp_choices
...
def read_residual_archive(archive_path: Path) -> ResidualArchiveParts:
    ...
    semantic, carrier, hpac, warp_choices = _decode_models(models)
...
def decode_production_tokens(
    ...
    with torch.inference_mode():
        ...
        for frame in range(runtime.N):
            ...
            if frame:
                # encoder-selected warp of frame t-1 (identity when the choice is 0)
                previous = warp_torch(previous, int(parts.warp_choices[frame]))
