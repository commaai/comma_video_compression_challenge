...
Usage:
    uv run python experiments/arithmetic_coder/encode_seg_c2split_a1_purepy.py --peel 4 --roundtrip
"""
...
def main() -> None:
    ...
    out_path = Path(args.out) if args.out else repo / "experiments/scoping_20260417/seg_targets_a1.bin"
    ...
    # compare against the trpp (task #14) shipped seg_targets.bin if present
    trpp_ref = repo / "submissions/qat_int8_bs32_segc4split_hc28_poslaplace_corrint8_ste_b_step81_mtrpp/archive/seg_targets.bin"
    if trpp_ref.exists():
        a_bytes = trpp_ref.stat().st_size
        delta = len(blob) - a_bytes
        print(f"  vs trpp seg_targets.bin ({a_bytes:,} B): {delta:+,} B")
