...
def configure_torch_threads() -> None:
  threads_text = os.environ.get("SHIFTAV1_TORCH_THREADS") or os.environ.get("OMP_NUM_THREADS")
  if not threads_text:
    return
...
def sidechannel_pose_weight() -> float:
  return float(os.environ.get("SHIFTAV1_SIDECHANNEL_POSE_WEIGHT", "6.3"))


def sidechannel_seg_weight() -> float:
  return float(os.environ.get("SHIFTAV1_SIDECHANNEL_SEG_WEIGHT", "100.0"))


def sidechannel_score_mode() -> str:
  return os.environ.get("SHIFTAV1_SIDECHANNEL_SCORE_MODE", "linear").strip().lower()
...
def combined_metric_score(pose: torch.Tensor, seg: torch.Tensor) -> torch.Tensor:
  ...
  raise SystemExit(f"unsupported SHIFTAV1_SIDECHANNEL_SCORE_MODE: {mode}")
...
def parse_sat_candidates() -> list[int]:
  text = os.environ.get("SHIFTAV1_SIDECHANNEL_SAT_CANDIDATES", "-6,-3,0,3,6")
...
def parse_shift_candidates() -> list[int]:
  text = os.environ.get("SHIFTAV1_SIDECHANNEL_SHIFT_CANDIDATES", "-2,-1,0,1,2")
  values = [int(round(float(item.strip()))) for item in text.replace(";", ",").split(",") if item.strip()]
  if not values:
...
def metric_y_gradient_values(
  ...
  steps = max(1, env_int("SHIFTAV1_SIDECHANNEL_GRAD_STEPS", 12))
  lr = env_float("SHIFTAV1_SIDECHANNEL_GRAD_LR", 1.5)
  pose_weight = env_float("SHIFTAV1_SIDECHANNEL_GRAD_POSE_WEIGHT", 6.3)
  seg_ce_weight = env_float("SHIFTAV1_SIDECHANNEL_GRAD_SEG_CE_WEIGHT", 0.02)
  refine_radius = max(0, env_int("SHIFTAV1_SIDECHANNEL_GRAD_REFINE_RADIUS", 1))
