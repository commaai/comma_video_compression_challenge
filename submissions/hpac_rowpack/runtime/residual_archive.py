...
def read_residual_archive(archive_path: Path) -> ResidualArchiveParts:
    ...
    if archive_path.stat().st_size > 2 * (1 << 20):
        raise ResidualArchiveError("archive exceeds fixed-codec size limit")
    with zipfile.ZipFile(archive_path) as archive:
        ...
        if archive.getinfo("p").file_size > (1 << 20):
            raise ResidualArchiveError("payload exceeds fixed-codec size limit")
    ...
    try:
        ...
        models = decoder.decompress(outer, max_length=1 << 20)
    ...
    # Our lossless wrapper restores the exact upstream model container before
    # the inherited semantic, carrier, and HPAC parsers observe its contents.
    if models.startswith(rowpack.MAGIC):
        models = rowpack.unpack_models(models)
