...
sys.path.insert(0, str(REPO / "experiments/arithmetic_coder"))
...
def feature_frame(seg_fi: np.ndarray, seg_prev_prev_prev: np.ndarray | None, name: str) -> np.ndarray:
    ...
    seg_prev_prev_prev  : None if fi < 3, else the (t-3) frame.
    """
    h, w = seg_fi.shape
...
def build_m10_counts(
    ...
    for fi in range(2, n_pairs):
        ...
        ext_ctx = m7_ctx
        for name in feats:
            ppp = seg[fi - 3].astype(np.int64) if (fi >= 3 and name == "prev_prev_prev") else None
            feat = feature_frame(frame, ppp, name)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="source seg_targets.bin (shipped a1 format or lzma fallback)",
    )
    ...
    out_path = Path(args.out) if args.out else repo / "experiments/seg_ctw/seg_targets_m10.bin"
    ...
    try:
        sys.path.insert(
            0,
            str(repo / "submissions/qat_int8_bs32_segc4split_hc28_poslaplace_corrint8_ste_b_step81_ma1"),
        )
