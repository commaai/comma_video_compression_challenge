...
from hpac_integer import IntegerHPAC, exact_power_of_two, integer_activation, requantize
...
class SparseIntegerHPAC:
    ...
    def _build_plan(self, group: int, device) -> GroupPlan:
        ...
        b1_gather = _gather_map(b1_positions, h_positions, self.b1_offsets, self.patch)
        b2_gather = _gather_map(targets, b1_positions, self.b2_offsets, self.patch)
        ...
        return GroupPlan(
            ...
            h_positions=torch.tensor(h_positions, dtype=torch.long, device=device),
            b1_gather=torch.tensor(b1_gather, dtype=torch.long, device=device),
            b2_gather=torch.tensor(b2_gather, dtype=torch.long, device=device),
            output_order=torch.tensor(output_order, dtype=torch.long, device=device),
        )
    ...
    def _apply_codes(value, module):
        ...
        if exponent is not None:
            value = value * exact_power_of_two(exponent).view(1, 1, -1)
    ...
    def _conv_a(self, inputs, positions):
        ...
        row = (
            positions[:, 0, None]
            + 3
            + torch.tensor([dy for dy, _ in self.a_offsets], device=inputs.device)
        )
        col = (
            positions[:, 1, None]
            + 3
            + torch.tensor([dx for _, dx in self.a_offsets], device=inputs.device)
        )
    ...
    def selected_logits(self, current, context, group: int):
        ...
        one_hot = (
            F.one_hot(current, num_classes=self.model.num_classes)
            .permute(0, 3, 1, 2)
            .float()
        )
        ...
        hidden = requantize(
            self._conv_a(inputs, plan.h_positions),
            1,
        ...
        hidden = integer_activation(
            requantize(
                self._depthwise(hidden, plan.b1_gather, self.model.conv_b1),
                3,
                -self.model.activation_bound,
                self.model.activation_bound,
            ),
        ...
        hidden = integer_activation(
            requantize(
                self._depthwise(hidden, plan.b2_gather, self.model.conv_b2),
                3,
                -self.model.activation_bound,
                self.model.activation_bound,
            ),
        ...
        logits = requantize(self._apply_codes(logits, head), 3, -32768, 32767) / 8.0
