...
UPSCALE_MODE = os.environ.get("SHIFTAV1_UPSCALE_MODE", "bicubic")
CAMERA_UPSCALE_MODE = os.environ.get("SHIFTAV1_CAMERA_UPSCALE_MODE", UPSCALE_MODE)
INVERSE_ITERS = int(os.environ.get("SHIFTAV1_INVERSE_ITERS", "2"))
INVERSE_GAIN = float(os.environ.get("SHIFTAV1_INVERSE_GAIN", "1.0"))
INVERSE_BASE_MODE = os.environ.get("SHIFTAV1_INVERSE_BASE_MODE", "bicubic")
EDGE_BASE_MODE = os.environ.get("SHIFTAV1_EDGE_BASE_MODE", "bicubic")
EDGE_AMOUNT = float(os.environ.get("SHIFTAV1_EDGE_AMOUNT", "0.12"))
EDGE_LOW = float(os.environ.get("SHIFTAV1_EDGE_LOW", "2.0"))
EDGE_HIGH = float(os.environ.get("SHIFTAV1_EDGE_HIGH", "16.0"))
EDGE_KERNEL = int(os.environ.get("SHIFTAV1_EDGE_KERNEL", "3"))
EDGE_FLAT_BLUR = float(os.environ.get("SHIFTAV1_EDGE_FLAT_BLUR", "0.0"))
POST_UNSHARP_AMOUNT = float(os.environ.get("SHIFTAV1_POST_UNSHARP_AMOUNT", "0.0"))
POST_UNSHARP_KERNEL = int(os.environ.get("SHIFTAV1_POST_UNSHARP_KERNEL", "3"))
POST_UNSHARP_THRESHOLD = float(os.environ.get("SHIFTAV1_POST_UNSHARP_THRESHOLD", "0.0"))
POST_BLUR_BLEND = float(os.environ.get("SHIFTAV1_POST_BLUR_BLEND", "0.0"))
POST_SATURATION = os.environ.get("SHIFTAV1_POST_SATURATION", "1.0")
POST_LUMA_GAIN = os.environ.get("SHIFTAV1_POST_LUMA_GAIN", "1.0")
POST_LUMA_BIAS = os.environ.get("SHIFTAV1_POST_LUMA_BIAS", "0.0")
POST_RGB_GAIN = os.environ.get("SHIFTAV1_POST_RGB_GAIN", "1.0,1.0,1.0")
POST_RGB_BIAS = os.environ.get("SHIFTAV1_POST_RGB_BIAS", "0.0,0.0,0.0")
POST_SCHEDULE_FRAMES = int(os.environ.get("SHIFTAV1_POST_SCHEDULE_FRAMES", "10"))
POSTPROCESS_SEGMENTS = {
  int(item)
  for item in os.environ.get("SHIFTAV1_POSTPROCESS_SEGMENTS", "").replace(",", " ").split()
  if item.strip()
}
TEMPORAL_BLEND = float(os.environ.get("SHIFTAV1_TEMPORAL_BLEND", "0.025"))
TEMPORAL_MODE = os.environ.get("SHIFTAV1_TEMPORAL_MODE", "fixed")
TEMPORAL_DIFF_LOW = float(os.environ.get("SHIFTAV1_TEMPORAL_DIFF_LOW", "4.0"))
TEMPORAL_DIFF_HIGH = float(os.environ.get("SHIFTAV1_TEMPORAL_DIFF_HIGH", "18.0"))
TEMPORAL_MIN_FACTOR = float(os.environ.get("SHIFTAV1_TEMPORAL_MIN_FACTOR", "0.0"))
TEMPORAL_MAX_FACTOR = float(os.environ.get("SHIFTAV1_TEMPORAL_MAX_FACTOR", "1.0"))
TEMPORAL_KERNEL = int(os.environ.get("SHIFTAV1_TEMPORAL_KERNEL", "5"))
TEMPORAL_MOTION_RADIUS = int(os.environ.get("SHIFTAV1_TEMPORAL_MOTION_RADIUS", "2"))
TEMPORAL_MOTION_DOWNSAMPLE = int(os.environ.get("SHIFTAV1_TEMPORAL_MOTION_DOWNSAMPLE", "2"))
TEMPORAL_SCHEDULE_FRAMES = int(os.environ.get("SHIFTAV1_TEMPORAL_SCHEDULE_FRAMES", "10"))
SIDECHANNEL_FEEDBACK = os.environ.get("SHIFTAV1_SIDECHANNEL_FEEDBACK", "0") in {"1", "true", "yes", "on"}
POSTFILTER_DEVICE = os.environ.get("SHIFTAV1_POSTFILTER_DEVICE", "auto")
POSTFILTER_SIZE = os.environ.get("SHIFTAV1_POSTFILTER_SIZE", "")
POSTFILTER_RESIZE_MODE = os.environ.get("SHIFTAV1_POSTFILTER_RESIZE_MODE", "bilinear")
POSTFILTER_PATH = os.environ.get("SHIFTAV1_POSTFILTER_PATH", "")
PAIR_ASYM_MODE_ENV = os.environ.get("SHIFTAV1_PAIR_ASYM_MODE")
PAIR_ASYM_BLEND = float(os.environ.get("SHIFTAV1_PAIR_ASYM_BLEND", "0.5"))
...
def camera_upscale_mode(segment_idx: int) -> str:
  segment_key = f"SHIFTAV1_CAMERA_UPSCALE_SEGMENT{segment_idx}_MODE"
  if segment_key in os.environ:
    return os.environ[segment_key]
  if segment_idx >= 1 and "SHIFTAV1_CAMERA_UPSCALE_MODE" not in os.environ:
    return "eval-inverse"
  return os.environ.get(f"SHIFTAV1_CAMERA_UPSCALE_SEGMENT{segment_idx}_MODE", CAMERA_UPSCALE_MODE)
...
def segment_env(segment_idx: int, suffix: str, default: str, final_segment_idx: int | None = None) -> str:
  if suffix == "BLEND" and segment_idx >= 1 and "SHIFTAV1_TEMPORAL_BLEND" not in os.environ:
    ...
    if segment_idx == 1:
      ...
      default = "0.015"
  return os.environ.get(f"SHIFTAV1_TEMPORAL_SEGMENT{segment_idx}_{suffix}", default)
...
def scheduled_segment_env(
  ...
  schedule_key = f"SHIFTAV1_TEMPORAL_SEGMENT{segment_idx}_{suffix}_SCHEDULE"
  values = None
  if schedule_key in os.environ:
    values = [item.strip() for item in os.environ[schedule_key].replace(";", ",").split(",") if item.strip()]
  else:
    global_schedule_key = f"SHIFTAV1_TEMPORAL_{suffix}_SCHEDULE"
...
def post_env(segment_idx: int, suffix: str, default: str) -> str:
  key = f"SHIFTAV1_POST_SEGMENT{segment_idx}_{suffix}"
...
def scheduled_post_env(segment_idx: int, frame_in_segment: int, suffix: str, default: str) -> str:
  schedule_key = f"SHIFTAV1_POST_SEGMENT{segment_idx}_{suffix}_SCHEDULE"
  values = None
  if schedule_key in os.environ:
    values = [item.strip() for item in os.environ[schedule_key].replace(";", ",").split(",") if item.strip()]
  else:
    global_schedule_key = f"SHIFTAV1_POST_{suffix}_SCHEDULE"
    if global_schedule_key in os.environ:
      values = [item.strip() for item in os.environ[global_schedule_key].replace(";", ",").split(",") if item.strip()]
